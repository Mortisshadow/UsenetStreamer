...
- **NZBDav / InfiniDysk:** `NZBDAV_URL`, `NZBDAV_API_KEY`, WebDAV credentials, category controls, and history/cache options. Set `NZBDAV_BACKEND=auto` (default) to prefer InfiniDysk's native JSON directory API with WebDAV fallback. Supplying InfiniDysk's `FRONTEND_BACKEND_API_KEY` as `INFINIDYSK_FRONTEND_API_KEY` additionally enables its optimized `/view` streaming route through the UsenetStreamer proxy; keep this secret private.
